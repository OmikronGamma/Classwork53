...
class VetClinicRouter:
    def db_for_read(self, model, **hints):
        return 'vetclinic'  # always read from 'vetclinic' database

    def db_for_write(self, model, **hints):
        return 'vetclinic'  # always write to 'vetclinic' database

    def allow_relation(self, obj1, obj2, **hints):
        return True  # always allow relationships

    def allow_migrate(self, db, app_label, model_name=None, **hints):
        return db == 'vetclinic'  # only allow migrations on 'vetclinic' database
